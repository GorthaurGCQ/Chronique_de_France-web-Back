...
export default function RegionFriseEtCards({
  categories,
}: {
    ...
    <>
        ...
        <div className={styles.catNav}>
          ...
          <div className={styles.catCenter} ref={dropdownRef}>
            ...
            >
              ...
              <span
                className={`${styles.catChevron} ${dropdownOpen ? styles.catChevronOpen : ""}`}
              >
                ▾
              </span>
            </button>
      ...
      {currentCat.cards.length === 0 ? (
        <div className={styles.catEmpty}>
          ...
          <p className={styles.catEmptyHint}>
            Revenez bientôt — de nouveaux contenus seront ajoutés prochainement.
          </p>
        </div>
      ) : filteredCards.length === 0 ? (
        <p className={styles.friseEmpty}>
          Aucun contenu pour cette période — d&apos;autres ressources seront
          ajoutées prochainement.
        </p>
      ) : (
        <div
          className={styles.grid}
          key={`${catIdx}-${epoqueFilter ?? "all"}`}
        >
